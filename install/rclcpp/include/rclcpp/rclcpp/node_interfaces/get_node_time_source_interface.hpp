 ...
 *
 * It's able to get a std::shared_ptr to a NodeTimeSourceInterface so long as the class
 * has a method called ``get_node_time_source_interface()`` which returns one.
 */
...
template<
  typename NodeType,
  typename std::enable_if<has_node_time_source_interface<
...
inline
std::shared_ptr<rclcpp::node_interfaces::NodeTimeSourceInterface>
get_node_time_source_interface(NodeType && node)
{
  // Forward pointers to detail implementation directly.
  return detail::get_node_time_source_interface_from_pointer(node);
}
...
get_node_time_source_interface(
  std::shared_ptr<rclcpp::node_interfaces::NodeTimeSourceInterface> & node_interface)
{
