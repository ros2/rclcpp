...
TEST_F(TestNodeGraph, get_action_client_names_and_types_by_node_rcl_error)
{
  ...
  auto mock = mocking_utils::patch_and_return(
    "lib:rclcpp", rcl_action_get_client_names_and_types_by_node, RCL_RET_ERROR);
  auto mock_names_fini = mocking_utils::patch_and_return(
    "lib:rclcpp", rcl_names_and_types_fini, RCL_RET_ERROR);
  RCLCPP_EXPECT_THROW_EQ(
    node_graph()->get_action_client_names_and_types_by_node(node_name, node_namespace),
    std::runtime_error(
      "failed to get action service names and types by node: error not set"));
}
...
{
  auto handle_goal = [](const GoalUUID &, std::shared_ptr<const test_msgs::msg::Empty::Goal>){
    return rclcpp::GoalResponse::ACCEPT_AND_DEFER;
  };
  auto handle_cancel = [](std::shared_ptr<test_msgs::msg::Empty> {
    return rclcpp::CancelResponse::ACCEPT;
  };
...
TEST_F(TestNodeGraph, get_action_server_names_and_types_by_node_rcl_error)
{
  ...
  auto mock = mocking_utils::patch_and_return(
    "lib:rclcpp", rcl_action_get_server_names_and_types_by_node, RCL_RET_ERROR);
  auto mock_names_fini = mocking_utils::patch_and_return(
    "lib:rclcpp", rcl_names_and_types_fini, RCL_RET_ERROR);
  RCLCPP_EXPECT_THROW_EQ(
