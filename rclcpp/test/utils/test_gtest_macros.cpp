...
TEST(TestGTestMacros, non_standard_types) {
  EXPECT_THROW_EQ(throw 0, 0);

  EXPECT_THROW_EQ(throw 42, 42);

  EXPECT_THROW_EQ(throw std::string("some string"), std::string("some string"));

  EXPECT_THROW_EQ(throw NonStandardThrowable(), NonStandardThrowable());

  ASSERT_THROW_EQ(throw 0, 0);

  ASSERT_THROW_EQ(throw 42, 42);

  ASSERT_THROW_EQ(throw std::string("some string"), std::string("some string"));

  ASSERT_THROW_EQ(throw NonStandardThrowable(), NonStandardThrowable());
}
...
TEST(TestGTestMacros, rclcpp_exceptions) {
  rcutils_error_state_t rcl_error_state = {"this is some error message", __FILE__, __LINE__};
  {
    ...
    auto actual =
      rclcpp::exceptions::RCLError(RCL_RET_ERROR, &rcl_error_state, "exception_prefix");
    EXPECT_THROW_EQ(throw actual, expected);
    ASSERT_THROW_EQ(throw actual, expected);
  }
  {
    auto expected =
