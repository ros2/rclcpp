...
TEST(TestNodeOptions, ros_args_only)
{
  ...
  auto options = rclcpp::NodeOptions(allocator).arguments(
    {"--ros-args", "-r", "__node:=some_node", "-r", "__ns:=/some_ns"});
  ...
  EXPECT_EQ(
    RCL_RET_OK,
    rcl_remap_node_name(&rcl_options->arguments, nullptr, "my_node", allocator, &node_name));
...
TEST(TestNodeOptions, ros_args_and_non_ros_args)
{
  ...
  EXPECT_EQ(
    RCL_RET_OK, rcl_arguments_get_unparsed(&rcl_options->arguments, allocator, &output_indices));
...
{
  rcl_allocator_t allocator = rcl_get_default_allocator();
  auto options = rclcpp::NodeOptions(allocator).arguments({"--ros-args", "-r", "foo:="});

  EXPECT_THROW(options.get_rcl_node_options(), rclcpp::exceptions::RCLInvalidROSArgsError);

  options.arguments({"--ros-args", "-r", "foo:=bar", "not-a-ros-arg"});
  EXPECT_THROW(options.get_rcl_node_options(), rclcpp::exceptions::UnknownROSArgsError);
}
