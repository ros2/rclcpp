...
TEST_P(TestPublisherSubscriptionSerialized, publish_serialized_generic)
{
  ...
  TestParameters parameters = GetParam();
  {
    ...
    auto publisher = rclcpp::create_generic_publisher<rclcpp::SerializedMessage>(
      node,
      "/topic",
    ...
    auto sub_gen_serialized = rclcpp::create_generic_subscription<rclcpp::SerializedMessage>(
      node,
      "/topic",
      *rosidl_typesupport_cpp::get_message_type_support_handle<test_msgs::msg::Strings>(),
      rclcpp::QoS(10),
    ...
    for (size_t i = 0; i < parameters.runs; i++) {
      ...
      publisher->publish(std::make_unique<rclcpp::SerializedMessage>(std::move(msg0)));
    }
    rclcpp::spin_some(node);
    rclcpp::sleep_for(offset);
