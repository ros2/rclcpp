...
#define RCL_PARAM_DOMAIN_ID_RULE "__domain_id:="
  ...
  notify_guard_condition_is_valid_(false)
{
  ...
  size_t domain_id = 0;
  bool domain_by_args = false;
  // Check if any of the arguments is the domain_id
  for (auto arg : arguments)
  {
    if (strncmp(RCL_PARAM_DOMAIN_ID_RULE, arg.c_str(), strlen(RCL_PARAM_DOMAIN_ID_RULE)) == 0)
    {
        domain_id = stoul(arg.substr(strlen(RCL_PARAM_DOMAIN_ID_RULE)));
        domain_by_args = true;
        break;
    }
  }
