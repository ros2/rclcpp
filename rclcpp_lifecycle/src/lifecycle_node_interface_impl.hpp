...
public:
  ...
  ~LifecycleNodeInterfaceImpl()
  {
    ...
    auto ret =
      rcl_lifecycle_state_machine_fini(&state_machine_, node_handle, &node_options->allocator);
    if (ret != RCL_RET_OK) {
  ...
  void init()
  {
    ...
    rcl_ret_t ret = rcl_lifecycle_state_machine_init(
      ...
      rosidl_typesupport_cpp::get_service_type_support_handle<GetAvailableTransitionsSrv>(), true,
    ...
    {  // change_state
      auto cb = std::bind(
        &LifecycleNodeInterfaceImpl::on_change_state, this, std::placeholders::_1,
    ...
    {  // get_state
      auto cb = std::bind(
        &LifecycleNodeInterfaceImpl::on_get_state, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
    ...
    {  // get_available_states
      auto cb = std::bind(
        &LifecycleNodeInterfaceImpl::on_get_available_states, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
    ...
    {  // get_available_transitions
      auto cb = std::bind(
        &LifecycleNodeInterfaceImpl::on_get_available_transitions, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
    ...
    {  // get_transition_graph
      auto cb = std::bind(
        &LifecycleNodeInterfaceImpl::on_get_transition_graph, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
      rclcpp::AnyServiceCallback<GetAvailableTransitionsSrv> any_cb;
      any_cb.set(std::move(cb));
  ...
  std::vector<Transition> get_available_transitions()
  {
    ...
    for (uint8_t i = 0; i < state_machine_.transition_map.transitions_size; ++i) {
      Transition transition(&state_machine_.transition_map.transitions[i]);
  ...
  rcl_ret_t change_state(
    std::uint8_t transition_id, LifecycleNodeInterface::CallbackReturn & cb_return_code)
  {
    if (rcl_lifecycle_state_machine_is_initialized(&state_machine_) != RCL_RET_OK) {
      RCUTILS_LOG_ERROR(
    ...
    if (
      ...
      RCUTILS_LOG_ERROR(
        "Failed to finish transition %u. Current state is now: %s", transition_id,
  ...
  LifecycleNodeInterface::CallbackReturn execute_callback(
  ...
  std::map<std::uint8_t, std::function<LifecycleNodeInterface::CallbackReturn(const State &)>>
    cb_map_;
