 ...
 *
 * It's able to get a std::shared_ptr to a NodeServicesInterface so long as the class
 * has a method called ``get_node_services_interface()`` which returns one.
 */
...
template<
  typename NodeType,
  typename std::enable_if<has_node_services_interface<
...
inline
std::shared_ptr<rclcpp::node_interfaces::NodeServicesInterface>
get_node_services_interface(NodeType && node)
{
  // Forward pointers to detail implementation directly.
  return detail::get_node_services_interface_from_pointer(node);
}
...
get_node_services_interface(
  std::shared_ptr<rclcpp::node_interfaces::NodeServicesInterface> & node_interface)
{
