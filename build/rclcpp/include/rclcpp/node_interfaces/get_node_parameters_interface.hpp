 ...
 *
 * It's able to get a std::shared_ptr to a NodeParametersInterface so long as the class
 * has a method called ``get_node_parameters_interface()`` which returns one.
 */
...
template<
  typename NodeType,
  typename std::enable_if<has_node_parameters_interface<
...
inline
std::shared_ptr<rclcpp::node_interfaces::NodeParametersInterface>
get_node_parameters_interface(NodeType && node)
{
  // Forward pointers to detail implementation directly.
  return detail::get_node_parameters_interface_from_pointer(node);
}
...
get_node_parameters_interface(
  std::shared_ptr<rclcpp::node_interfaces::NodeParametersInterface> & node_interface)
{
